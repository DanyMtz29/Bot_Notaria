# bot/pages/customers_cif_modal.py
...
class CustomersCifModal:
    ...
    def assert_open(self, timeout: int = 15):
        ...
        logger.debug("Modal 'Crear cliente con IdCIF' listo.")
        return content
    ...
    def _click_consult(self, content, timeout: int = 10):
        ...
        btn.click()
        logger.debug("Click en 'Consultar datos actualizados'.")
        ...
        while time.time() < deadline:
            try:
                ...
                if comp.is_displayed():
                    logger.debug("Detalle CIF cargado en el modal.")
                    return True
            except Exception:
                pass
            time.sleep(0.3)
        logger.warning("No apareció app-customers-detail-cif tras la consulta.")
        return False
